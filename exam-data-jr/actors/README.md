...
| `actors/`       | A Python directory that contains code for your Dagster repository                 |
| `actors_tests/` | A Python directory that contains tests for `actors`                      |
...
pyenv virtualenv $PYTHON_VERSION actors
pyenv activate actors
```

**Conda**
```bash
export PYTHON_VERSION=X.Y.Z
conda create --name actors python=PYTHON_VERSION
conda activate actors
```
...
Tests can be found in `actors_tests` and are run with the following command:

```bash
pytest actors_tests
```

As you create Dagster ops and graphs, add tests in `actors_tests/` to check that your
